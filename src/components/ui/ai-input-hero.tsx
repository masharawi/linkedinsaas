...
export function HeroWave({ className, style, extendLeftPx = 320, title = "De AI-tool voor consistente zichtbaarheid op LinkedIn.", subtitle = "Gebaseerd op patronen uit duizenden succesvolle LinkedIn-posts.", placeholder = "Waar wil je zichtbaar mee worden?", buttonText = "Genereer", onPromptSubmit }: HeroWaveProps) {
  ...
  useEffect(() => {
    ...
    waveScene.fog = null as unknown as THREE.Fog;
    ...
    function setupPointerTracking() {
      ...
      const readCoords = (e: PointerEvent | TouchEvent): { x: number; y: number } => {
        ...
        return t ? { x: t.clientX, y: t.clientY } : { x: mouse.x + rect.left, y: mouse.y + rect.top };
      };
    ...
    function onResize(newW: number, newH: number) {
      ...
      if (barCount !== currentBarCount) {
        ...
        const aXArr = aX.array as Float32Array;
        const aTArr = aT.array as Float32Array;

        for (let i = 0; i < barCount; i++) {
          const x = startX + FIXED_BAR_WIDTH / 2 + i * (FIXED_BAR_WIDTH + gap);
          const t = barCount > 1 ? i / (barCount - 1) : 0;
          aXArr[i] = aXArr[i + barCount] = x;
          aTArr[i] = aTArr[i + barCount] = t;
        }
        aX.needsUpdate = true;
        aT.needsUpdate = true;
      }
      ...
      clearTimeout(heavyResizeTimer ?? 0);
    ...
    const ticker = () => {
      ...
      const dt = ((gsap.ticker as { deltaRatio?: () => number }).deltaRatio?.() ?? 1) * (1 / 60);
